...
class NRackProfile:
    ...
    def __init__(self, r_params: dict, config: CompressorConfig):
        ...
        self.main_tip_center = np.array([-W1/2, -self.r1w_m])   # H₁J₁ arc (main rotor tip)
        self.gate_tip_center = np.array([+W1/2,  self.r1w_m])   # G₂H₂ arc (gate rotor tip)
        ...
        if not (0.13 <= r2_ratio <= 0.305):
            print(f"⚠️  Main tip radius: r2/r1w = {r2_ratio:.3f} outside [0.13-0.305]")
        if not (0.13 <= r4_ratio <= 0.305):
            print(f"⚠️  Gate tip radius: r4/r1w = {r4_ratio:.3f} outside [0.13-0.305]")
        
        # QC: Extreme case warnings
        if r2_ratio > 0.35:
            print(f"⚠️  Main tip radius very large: r2/r1w = {r2_ratio:.3f} > 0.35")
        if r4_ratio > 0.35:
            ...
            if hasattr(self, 'profile_points') and self.profile_points:
                ...
                if G is not None and H is not None and J is not None:
                    ...
                    if h_circle_residual > 1e-6:  # QC spec: < 1 µm
                        print(f"⚠️  H circle residual {h_circle_residual*1e6:.1f} µm > 1 µm limit")
                    if j_circle_residual > 1e-6:  # QC spec: < 1 µm  
                        print(f"⚠️  J circle residual {j_circle_residual*1e6:.1f} µm > 1 µm limit")
                    
                    # Check clearance gap (warnings only)
                    gh_distance = np.linalg.norm(G - H)
                    if gh_distance < 0.05e-3:  # 0.05 mm minimum clearance
                        print(f"⚠️  GH clearance {gh_distance*1e3:.3f}mm < 0.05mm minimum")
    ...
    def _solve_all_junctions_simultaneously(self):
        ...
        QC Enhancement: Precision-optimized solver for < 1µm residuals.
        """
    ...
    def _validate_qc_precision(self, solution: np.ndarray) -> bool:
        ...
        try:
            ...
            # QC requirement: < 1µm for circle constraints
    ...
    def _theta_from_rack(self, x_rack: float, y_rack: float, r_w: float, dy_dx: float) -> float:
        ...
        # Eq. 2.5: f(θ) = dy_dx*(r_w*θ - y_rack) - (r_w - x_rack) = 0
        def func(theta):
        ...
        if abs(theta) > MAX_THETA:
            raise ValueError(f"Theta sanity check failed: |θ| = {abs(theta)} > {MAX_THETA}")
    ...
    def _generate_gh_trochoid(self, g_point: np.ndarray, h_point: np.ndarray, num_points: int) -> np.ndarray:
        """
        Generate GH trochoid as true envelope of gate tip arc G₂H₂.
        ...
        """
        # Ensure minimum points for curvature visualization and QC requirement (≥ 200)
        num_points = max(num_points, 200)
        
        try:
            # Step 1: Generate gate tip arc G₂H₂ from actual rotor geometry
            # QC clarification: Gate tip uses r₄ circle, NOT EF root fillet center
            ...
            tip_radius = self.r4  # Gate tip radius (r₄) in meters
            ...
            # QC requirement: < 1e-8 * r₁w dimensionless for full fidelity
    ...
    def _generate_hj_trochoid(self, h_point: np.ndarray, j_point: np.ndarray, num_points: int) -> np.ndarray:
        """
        Generate HJ trochoid as true envelope of main tip arc H₁J₁.
        QC approved: Full fidelity implementation using envelope theory.
        ...
        try:
            # Step 1: Generate main tip arc H₁J₁ from actual rotor geometry  
            # QC clarification: Main tip uses r₂ circle, NOT JA root fillet center
            from ..utils.coordinate_transforms import compute_tip_arc_center_from_points
            
            # CRITICAL FIX: Compute actual tip arc center from the junction points H and J
            tip_radius = self.r2  # Main tip radius (r₂) in meters
    ...
    def _generate_segments(self):
        ...
        if USE_TRUE_TROCHOIDS:
            ...
            # Validate C¹ continuity at H (QC requirement)
    ...
    def _get_heuristic_initial_guess(self) -> np.ndarray:
        ...
        """
        # From the checklist (§4): Use direct, stable angle estimates.
